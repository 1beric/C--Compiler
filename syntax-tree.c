...
bool ActualsMatchFormals(symtabnode *fn, tnode *actuals)
{
       ...
       formals = formals->next, actuals = ListNext(actuals), n++)
  {
    ...
    if (t0 != t_Array && t1 != t_Array /* neither is an array */
        && (t0 == t_Int || t0 == t_Char) && (t1 == t_Int || t1 == t_Char))
    {
      continue;
    }
    else if (t0 == t_Array && t1 == t_Array)
    { /* both are arrays */
      if ((argNode->ntype == Stringcon && formals->elt_type == t_Char) || formals->elt_type == SymTabPtr(argNode)->elt_type)
      {
        continue;
      }
      else
