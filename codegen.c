/*
 * File: codegen.c
 * Author: originally by Saumya Debray, reworked and optimizations added by Brandon Erickson
...
static void register_allocation(Quad *qptr);
static void register_allocation_rec(CFGILink *link, BitVec *LiveNow);
static int set_variable_positions(Quad *curr_ins, int reset);
...
static int operands_equal(Operand *a, Operand *b)
{
    ...
    return a->val.stptr == b->val.stptr;
}
...
static void register_allocation_rec(CFGILink *link, BitVec *LiveNow)
{
    ...
    // PROCESS NEXT FIRST
    register_allocation_rec(link->next, LiveNow);
    ...
    case MOVE:
    case UMINUS_OP:
        if (ins->dst->val.stptr->type == t_Int || ins->dst->val.stptr->type == t_Char)
            x = ins->dst->val.stptr;
    ...
    case IF_GT:
        ...
        if (ins->src2->optype != INTEGER &&
            (ins->src2->val.stptr->type == t_Int || ins->src2->val.stptr->type == t_Char))
    ...
    if (x != NULL && x->scope != Global)
        bv_set(LiveNow, x->func_pos, 0);
    if (y != NULL && y->scope != Global)
...
static void dead_code_elimination(Quad *fn_head)
{
    ...
    while (chg)
    {
        ...
        while (bblist != NULL)
        {
            if (deadcode_rec(bblist->block->ilinks, bv_copy(bblist->block->out)))
                chg = 1;
            bblist = bblist->next;
        }
    }

    // change dead instructions to nops
    Quad *curr = fn_head;
    while (curr != NULL)
    {
        if (!curr->live)
    ...
    // reset variable positions
    set_variable_positions(fn_head, 1);
}
...
static int deadcode_rec(CFGILink *link, BitVec *LIVE)
{
    ...
    if (x != NULL && !bv_get(LIVE, x->func_pos) && x->scope != Global)
    {
        ins->live = 0;
        return 1;
    }

    if (x != NULL && ins->dst->optype != DEREF && x->scope != Global)
...
static int set_variable_positions(Quad *curr_ins, int reset)
{
    ...
    while (curr_ins != NULL)
    {
        ...
        case MOVE:
        case UMINUS_OP:
            if (curr_ins->src1->optype != INTEGER && (reset || curr_ins->src1->val.stptr->func_pos == -1))
                curr_ins->src1->val.stptr->func_pos = (reset ? -1 : numvars++);
            if ((reset || curr_ins->dst->val.stptr->func_pos == -1))
                curr_ins->dst->val.stptr->func_pos = (reset ? -1 : numvars++);
            break;
        case IF_EQ:
        case IF_NE:
        case IF_LE:
        ...
        case IF_GT:
            if (curr_ins->src1->optype != INTEGER && (reset || curr_ins->src1->val.stptr->func_pos == -1))
                curr_ins->src1->val.stptr->func_pos = (reset ? -1 : numvars++);
            if (curr_ins->src2->optype != INTEGER && (reset || curr_ins->src2->val.stptr->func_pos == -1))
                curr_ins->src2->val.stptr->func_pos = (reset ? -1 : numvars++);
            break;
        case PARAM:
        case RET:
            if (curr_ins->src1 != NULL && curr_ins->src1->optype != INTEGER && (reset || curr_ins->src1->val.stptr->func_pos == -1))
                curr_ins->src1->val.stptr->func_pos = (reset ? -1 : numvars++);
            break;
        case ADD_OP:
        case SUB_OP:
